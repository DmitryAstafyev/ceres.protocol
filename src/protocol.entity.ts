...
import { IPrimitiveType, PrimitiveTypes } from './injections/injection.types.primitive';
...
export class EntityType {
    ...
    public setAdvancedTypes(advancedTypes: {[key: string]: any}): Error[] | undefined {
        const errors: Error[] | undefined = this._getAdvancedTypeErrors(advancedTypes);
    ...
    public getType(prop: string, target: any): EEntityType | Error[] {
        ...
        switch (Tools.getTypeOf(target)) {
            ...
            case Tools.EPrimitiveTypes.object:
                ...
                const errors: Error[] = [];
                Object.keys(target).forEach((nestedProp: string) => {
    ...
    public getInjections(): string {
        ...
        Object.keys(this._types).forEach((alias: string) => {
            ...
            if (Tools.getTypeOf(type.implementation) === Tools.EPrimitiveTypes.function) {
                injections += (type.implementation as () => void).toString() + '\n';
            }
        });
        if (injections !== '') {
            injections =
    ...
    private _getAdvancedTypeErrors(advancedTypes: {[key: string]: any}): Error[] | undefined {
    ...
    private _getEnumError(target: string[]): null | Error {
